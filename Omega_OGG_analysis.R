...
OGG_omega_table <- 
  read.table("ALL_OGG_OMEGA.csv",
             header=FALSE,
             sep=",")

list_species <- scan("list_species",
                     what="character")

colnames(OGG_omega_table) <- c("OGG", "gene_name", "LB", "MLE", "UB", "dN", "dS")
...
OGG_omega_table <- 
  OGG_omega_table %>%
  dplyr::filter(! OGG %in% opsins_OGG)
...
all_gene_name <- OGG_omega_table %>% pull(gene_name) %>% unique()
OGG_omega_table_sp <- as.data.frame(NULL)
for(curr_species in list_species){
  ...
  curr_sp_table <- 
    OGG_omega_table %>%
    filter(gene_name %in% curr_sp_gene_names) %>%
    mutate(species = curr_species)
  
  OGG_omega_table_sp <- 
    rbind(OGG_omega_table_sp,
          curr_sp_table)
...
##### Prepare files for pGLS --------------------


mean_omega_per_sp_per_ogg_df <- 
  as.data.frame(
    OGG_omega_table_sp %>%
      group_by(OGG, species) %>%
      summarise(mean_omega = mean(MLE)))

mean_omega_per_sp_per_ogg_df_wide <- 
  as.data.frame(
    mean_omega_per_sp_per_ogg_df %>%
      pivot_wider(names_from = OGG, values_from = mean_omega))
...
#Remove OGG with less than 20 teleost species or with null variance
...
list_OGG_sup20 <- 
  mean_omega_per_sp_per_ogg_df %>%
  filter(species %in% list_teleost_sp) %>%
  group_by(OGG) %>%
  summarise(count = n()) %>%
  filter(count >= 20) %>%
  pull(OGG)
...
list_OGG_sup20_all_fish <- 
  mean_omega_per_sp_per_ogg_df %>%
  filter(species %in% list_all_sp) %>%
  group_by(OGG) %>%
  summarise(count = n()) %>%
  filter(count >= 20) %>%
  pull(OGG)
...
write(list_OGG_sup20_all_fish, "list_OGG_sup20_all_fish.txt")
...
list_OGG_sup20 <- 
  scan("list_OGG_sup20_all_fish.txt",
       what="character")
