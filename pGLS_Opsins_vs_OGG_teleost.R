...
#Remove opsins OGG from the OGG table
...
#caper_data_opsins_ecology_IQTREE <- 
#  comparative.data(phy = LG_species_tree_nodelabel, data = all_ospins_complete_wide_ecology,
#                   names.col = species, vcv = TRUE,
#                   na.omit = FALSE, warn.dropped = TRUE)
...
##### pGLS between opsins and OGG  -------------------


list_OGG <- 
  colnames(OGG_table_df_filtered %>%
             dplyr::select(- species))
list_OGG <- c(list_OGG[-1], list_OGG[1])



responses_variables <- 
  all_ospins_complete_wide %>% 
  dplyr::select(c(total_non_visual, total_visual, total_opsins))
...
for(curr_response in responses_variables){
  ...
  for(curr_predictor in list_OGG){
    ...
    variance <- var(curr_data_opsins %>% filter(! is.na(predictor)) %>% pull(predictor))
