...
blast_rslt <- blast_rslt %>% 
  filter(evalue <= 1e-10) %>%
  mutate(scaffold_length = end-start) %>% filter(scaffold_length > 50)
...
exonerate_rslt <- exonerate_rslt %>% distinct()
...
#exonerate_rslt_filtered %>% filter(seqnames=="groupI") %>% filter(start > 10738000) %>% filter(end < 10747784)

Best_hits_filtered <- as.data.frame(NULL) 

for(putative_protein_length in c(0.8,0.75,0.7,0.65)){ 
