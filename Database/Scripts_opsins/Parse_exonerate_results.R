...
blast_rslt <- read.table("Opsin_vs_Genome.blastn", header=FALSE, sep="\t")
...
blast_rslt <- blast_rslt %>% dplyr::select(sseqid, true_start, true_end, evalue, strand, length)
colnames(blast_rslt) <- c("seqnames", "start", "end", "evalue", "strand", "length")

#keep only blast with an evalue < 1e-05 
blast_rslt <- blast_rslt %>% filter(evalue <= 1e-05) %>% mutate(scaffold_length = end-start) %>% filter(scaffold_length > 50)
...
#blast_result_besthits <- blast_result_besthits %>% filter(width > 50) #keep only besthits longer than 80 nucleotides. Remove spurious blast results
...
exonerate_rslt <- exonerate_rslt %>% 
  mutate(renamed_scaff = str_replace(scaffold, ":", "-"))  %>% 
  separate(renamed_scaff, c("true_scaffold_name", "true_scaff_start", "true_scaff_stop"), "-") %>% 
  mutate(true_coord_start = as.numeric(true_scaff_start)+scaffold_start) %>% 
  mutate(true_coord_end = as.numeric(true_scaff_start)+scaffold_end) %>%
  distinct()
...
exonerate_rslt_filtered <- left_join(exonerate_rslt, blast_result_besthits, by = "seqnames", suffix=c("", "_b")) %>%
  filter(start_b >= start-150) %>%
  filter(end_b <= end+150) %>%
...
### add cases where the blast hit is longer than the exonerate hit

missing_exo <- left_join(exonerate_rslt, blast_result_besthits, by = "seqnames", suffix=c("", "_b")) %>%
  filter(start_b < start) %>%
  filter(end_b > end) %>%
  group_by(seqnames, start, end, intron_number, query, query_end, query_start, evalue, best_query, strand, exonerate_score) %>%
  count(name="n_hit") %>%
  ungroup()

exonerate_rslt_filtered <- rbind(exonerate_rslt_filtered, missing_exo)



#Lets parse the results

exonerate_rslt_filtered <- 
  as.data.frame(exonerate_rslt_filtered %>% 
                  mutate(query_length = query_end - query_start) %>% 
                  mutate(scaffold_length = end-start))



#Filter genes depending on the query length

exonerate_rslt_filtered <- left_join(exonerate_rslt_filtered, query_length, by="query")
exonerate_rslt_filtered <- 
  exonerate_rslt_filtered %>%
  mutate(perc_aligned = query_length/total_q_length)


Best_hits_filtered <- as.data.frame(NULL) 


for(putative_protein_length in c(1,0.95,0.9,0.85,0.8)){ 
  
  
  exonerate_rslt_filtered_round <- 
    exonerate_rslt_filtered %>% 
    filter(perc_aligned >= putative_protein_length)
  ...
  if(nrow(Best_hits_filtered) > 0){
    for (row in 1:nrow(Best_hits_filtered)) {
      ...
      exonerate_rslt_filtered_round <- 
        exonerate_rslt_filtered_round %>% 
        filter(!(seqnames == as.character(Best_hits_filtered[row, "seqnames"]) & start <= (	Best_hits_filtered[row, "start"]) & end >= (Best_hits_filtered[row, "start"])))
      
      exonerate_rslt_filtered_round <- 
        exonerate_rslt_filtered_round %>% 
        filter(!(seqnames == as.character(Best_hits_filtered[row, "seqnames"]) & start >= (	Best_hits_filtered[row, "start"]) & end <=(Best_hits_filtered[row, "end"])))
      
      exonerate_rslt_filtered_round <- 
        exonerate_rslt_filtered_round %>% 
        filter(!(seqnames == as.character(Best_hits_filtered[row, "seqnames"]) & start <= (	Best_hits_filtered[row, "end"]) & end >= (Best_hits_filtered[row, "end"])))
