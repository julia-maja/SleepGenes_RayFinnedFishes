...
Opsins_best_hits <- scan(file="Opsins_Regions.tsv", what="character")


Best_hits_filtered <- Best_hits_filtered %>% filter(samtools_name %in% Opsins_best_hits)


#remove regions of already found T1R genes
...
#Extend each regions 10000bp on three prime and 10000bp on five prime. Do not extend if there is a functional genes in this windows. If so, 
...
write(Best_hits_filtered$samtools_name, file="Potential_Opsins_regions.tsv")
